...
def on_test_start(**kwargs):
    ...
    params = Tools().get_csv_info('recordDetailTop1000.csv')
...
class StudyTest(HttpUser):
    ...
    def get_watched_media_count(self):
        ...
        url = "/questionRecordDetailApi/findQuestionRecordDetailByCustomerId"
        ...
        with self.client.post(url=url, params=params, catch_response=True) as response:
            if response.status_code == 200:
                ...
                if rst:
                    try:
                        if str(rst[0]['customerId']) == params['customerId']:
                            response.success()
                        else:
